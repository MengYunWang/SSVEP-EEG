...
range_cycles = 4; % define the cycles;
...
Neutral_power = zeros(EEG.nbchan,EEG.pnts,length(data1_name));
for ii = 1:length(data1_name);
    ...
    wavelet  = exp(2*1i*pi*frex.*wavtime) .* exp(-wavtime.^2./(2*s^2));
...
Happy_power = zeros(EEG.nbchan,EEG.pnts,length(data1_name));
...
N2H_power = zeros(EEG.nbchan,EEG.pnts,length(data1_name));
...
H2N_power = zeros(EEG.nbchan,EEG.pnts,length(data1_name));
...
Static_power = zeros(EEG.nbchan,EEG.pnts,length(data1_name));
for ii = 1:length(data1_name);
    EEG = pop_loadset('filename',data1_name(ii).name,'filepath','F:\face-random\Preprocessing\Conditions\Condition1_Neutral\');
...
Dynamic_power = zeros(EEG.nbchan,EEG.pnts,length(data1_name));
for ii = 1:length(data1_name);
    EEG = pop_loadset('filename',data3_name(ii).name,'filepath','F:\face-random\Preprocessing\Conditions\Condition3_N2H\');
    ERP_temp1 = squeeze(nanmean(EEG.CSD,3));
    EEG = pop_loadset('filename',data4_name(ii).name,'filepath','F:\face-random\Preprocessing\Conditions\Condition4_H2N\');
...
baseline_power_sta =( nanmean(Neutral_power(:,baseindex(1):baseindex(2),:),2) + nanmean(Happy_power(:,baseindex(1):baseindex(2),:),2))./2;
baseline_power_dyn = ( nanmean(N2H_power(:,baseindex(1):baseindex(2),:),2) + nanmean(H2N_power(:,baseindex(1):baseindex(2),:),2))./2;
baseline_power = (nanmean(Static_power(:,baseindex(1):baseindex(2),:),2) + nanmean(Dynamic_power(:,baseindex(1):baseindex(2),:),2)./2; 
%         baseline_power_neutral = nanmean (Neutral_power(:,:,baseindex(1):baseindex(2),:),3);
        Neutral_dB = 10.*log10((Neutral_power)./repmat(baseline_power_sta,1,length(time2save),1));
        
%         baseline_power_happy = nanmean(Happy_power(:,:,baseindex(1):baseindex(2),:),3);
        Happy_dB = 10.*log10((Happy_power)./repmat(baseline_power_sta,1,length(time2save),1));
        
%         baseline_power_n2h = nanmean(N2H_power(:,:,baseindex(1):baseindex(2),:),3);
        N2H_dB = 10.*log10((N2H_power)./repmat(baseline_power_dyn,1,length(time2save),1));
        
%         baseline_power_h2n = nanmean(H2N_power(:,:,baseindex(1):baseindex(2),:),3);
        H2N_dB = 10.*log10((H2N_power)./repmat(baseline_power_dyn,1,length(time2save),1));
        
%         baseline_power_static = nanmean(Static_power(:,:,baseindex(1):baseindex(2),:),3);
        Static_dB = 10.*log10((Static_power)./repmat(baseline_power,1,length(time2save),1));
        
%         baseline_power_dynamic = nanmean(Dynamic_power(:,:,baseindex(1):baseindex(2),:),3);
        Dynamic_dB = 10.*log10((Dynamic_power)./repmat(baseline_power,1,length(time2save),1));
